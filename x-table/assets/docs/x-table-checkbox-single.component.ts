...
@Component({
  selector: 'x-table-checkbox-single',
  template: `
    <p>
      <button nz-button nzType="default" (click)="getCheckedRows()">读取选中的数据</button>
    </p>
    <x-table [columns]="columns" [url]="'/xtable/student/list'" [chkMultiple]="false" [(checkedRows)]="checkedRows" (checkedChange)="checkedChange($event)"></x-table>
  `,
  styles: []
})
export class XTableCheckboxSingleComponent implements OnInit {
  checkedRows = [];
